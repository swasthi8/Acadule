...
export function TransparentTimetableView({ sections, periods, timetables, onDone, onNotify }: Props) {
  ...
  return <div className="grid gap-6">
    ...
    {selected && <>
      ...
      <Panel title="Shared teacher conflict report" eyebrow="Teacher + day + period"><div className={`rounded-xl border px-4 py-3 text-sm ${selected.teacherConflicts.length ? "border-red-500/70 bg-red-950/70 text-red-100" : "border-emerald-400/30 bg-emerald-400/10 text-emerald-200"}`}>{selected.teacherConflicts.length ? `${selected.teacherConflicts.length} teacher collision(s) detected` : "PASS · No teacher appears in two sections at the same day and period."}</div>{selected.teacherConflicts.length > 0 && <Table headers={["Teacher", "Day", "Period", "Sections"]}>{selected.teacherConflicts.map((conflict) => <tr className="border-t border-slate-800" key={conflict.key}><td className="py-3 text-white">{conflict.teacherName}</td><td className="py-3 text-slate-400">{conflict.day}</td><td className="py-3 text-slate-400">P{conflict.periodNumber}</td><td className="py-3 text-slate-400">{conflict.sections.join(", ")}</td></tr>)}</Table>}</Panel>
    </>}
    <Panel title="Saved timetables" eyebrow="Existing drafts and publications"><Table headers={["Version", "Status", "Entries", "Last updated", "Action"]}>{timetables.map((item) => <tr className="border-t border-slate-800" key={item.id}><td className="py-4 font-mono text-cyan-200">v{item.version}</td><td className="py-4"><span className="rounded-full bg-amber-400/10 px-2.5 py-1 text-xs text-amber-300">{item.status}</span></td><td className="py-4 text-slate-400">{item.entries}</td><td className="py-4 text-slate-500">{new Date(item.updatedAt).toLocaleString()}</td><td className="py-4 text-right"><button className="rounded-lg border border-red-500/70 bg-red-950/70 px-3 py-2 text-xs font-semibold text-red-200 shadow-sm shadow-red-950/20 transition hover:border-red-400 hover:bg-red-900/80 hover:text-red-100" onClick={() => deleteTimetable(item)}>Delete</button></td></tr>)}</Table>{!timetables.length && <p className="rounded-xl border border-dashed border-slate-700 px-4 py-8 text-center text-sm text-slate-500">No timetable drafts yet. Generate one to see the weekly grid.</p>}</Panel>
  </div>;
}
