...
export default async function TeacherPage() {
  ...
  const teacher = await prisma.teacher.findUnique({ where: { userId: session.user.id }, select: { id: true, isAvailable: true } });
  if (!teacher) return <TeacherPortal name={session.user.name ?? "Teacher"} email={session.user.email ?? ""} isAvailable={true} periods={[]} workingDays={[]} entries={[]} />;
  ...
  return <TeacherPortal name={session.user.name ?? "Teacher"} email={session.user.email ?? ""} isAvailable={teacher.isAvailable} periods={periods} workingDays={workingDays} entries={entries.map((entry) => ({ id: entry.id, subject: entry.subject.name, sectionId: entry.sectionId, section: entry.section.name, className: entry.section.class.name, room: entry.room.name, day: entry.day, periodId: entry.periodId, periodNumber: entry.period.periodNumber, startTime: entry.period.startTime, endTime: entry.period.endTime }))} />;
}
