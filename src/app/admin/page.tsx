...
export default async function AdminPage() {
  ...
  const [teachers, students, classes, sections, subjects, rooms, periods, assignments, sectionSubjects, teacherAvailability, sectionAvailability, roomAvailability] = await Promise.all([
    ...
    prisma.class.findMany({ include: { sections: { include: { defaultRoom: true }, orderBy: { name: "asc" } } }, orderBy: { name: "asc" } }),
    prisma.section.findMany({ include: { class: true, defaultRoom: true }, orderBy: [{ class: { name: "asc" } }, { name: "asc" }] }),
  ...
  return (
    <AdminPortal
      ...
      data={{
        ...
        classes: classes.map((record) => ({ ...record, sections: record.sections.map((section) => ({ ...section, defaultRoomName: section.defaultRoom?.name ?? "" })) })),
        sections: sections.map((section) => ({ id: section.id, name: section.name, capacity: section.capacity, classId: section.classId, className: section.class.name, defaultRoomId: section.defaultRoomId, defaultRoomName: section.defaultRoom?.name ?? "" })),
        subjects,
        rooms,
        periods,
        availability: [
          ...teacherAvailability.filter((item) => item.status === "UNAVAILABLE").map((item) => ({ id: item.id, kind: "teacher" as const, entityId: item.teacherId, resourceName: item.teacher.user.name ?? item.teacher.employeeCode, day: item.day, periodId: item.periodId, periodNumber: item.period.periodNumber, startTime: item.period.startTime, status: item.status })),
          ...roomAvailability.filter((item) => item.status === "UNAVAILABLE").map((item) => ({ id: item.id, kind: "room" as const, entityId: item.roomId, resourceName: item.room.name, day: item.day, periodId: item.periodId, periodNumber: item.period.periodNumber, startTime: item.period.startTime, status: item.status })),
          ...sectionAvailability.filter((item) => item.status === "UNAVAILABLE").map((item) => ({ id: item.id, kind: "section" as const, entityId: item.sectionId, resourceName: `${item.section.class.name} / ${item.section.name}`, day: item.day, periodId: item.periodId, periodNumber: item.period.periodNumber, startTime: item.period.startTime, status: item.status })),
        ],
